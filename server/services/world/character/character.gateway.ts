...
export class CharacterGateway {
  ...
  async sendCharacters() {
    ...
    for (let player of players) {
      await this.character.characterOnline(
        player.accountId,
        player.socketId
      );
    }
  ...
  async createCharacter(
    ...
    try {
      ...
      if (player) {
        let character: Character = await this.service.createCharacter(
          player.accountId,
          data.name,
          data.gender
        );
        if (character) {
          client.emit(
            GetCharacters.event,
            await this.service.getCharacters(player.accountId)
          );
          client.emit(
            CharacterCreated.event,
            new CharacterCreated(character.world, character.id)
          );
          return character;
        }
      }
      ...
      client.emit(
        CharacterNotCreated.event,
        new CharacterNotCreated(e.response)
      );
      return null;
    }
  }
