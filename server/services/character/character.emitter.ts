...
export class CharacterEmitter {
    ...
    characterLoggedIn(id: number, gender: 'male' | 'female', world:string, name: string) {
        this.client.emit(WORLD_PREFIX + CharacterLoggedIn.event, new CharacterLoggedIn(id, name, world, gender));
    }

    characterLoggedOut(id: number, name: string, world:string) {
        this.client.emit(WORLD_PREFIX + CharacterLoggedOut.event, new CharacterLoggedOut(id, name, world));
    }
}
